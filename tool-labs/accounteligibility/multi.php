...
$users = explode(',', $backend->get('users') ?: $backend->getRouteValue(1) ?: '');
$wiki = $backend->get('wiki', null);
$backend->profiler->start('init engine');
$engine = new AccountEligibilityEngine($backend, $users[0], $event, $wiki);
...
############################
echo '
<form action="', $backend->url('/accounteligibility/multi.php'), '" method="get">
    <label for="user">User:</label>
    <textarea name="users" id="users" value="', $backend->formatValue($engine->user->name), '" ></textarea> at
...
foreach ($users as $user) {
    ...
    while ($engine->user->name) {
        ...
        if ($engine->wiki && !$engine->event->allowsDatabase($engine->wiki->dbName)) {
            $allowedDomains = array_map(function ($dbName) {
                global $engine;
                return $engine->wikis[$dbName]->domain;
            }, $engine->event->onlyDatabaseNames);
